...
// Copyright (c) 2013-2015 84 Developers
...
uint256 hashGenesisBlock("0x4eca3cfb295cb8f84c6dfb52de099c508f18ac5233a95cc10324999fa1898843");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // 84: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "84 Signed Message:\n";
...
{
    int64 nSubsidy = 0.000084 * COIN;
    if(nHeight < 419)
    {
       nSubsidy = 0.00084 * COIN;
	}
    if(nHeight == 1)
    {
       nSubsidy = 0.84 * COIN;
	}
    if(nHeight == 840) // yay its 840 :) Time for a smoke
    {
       nSubsidy = 0.00084 * COIN;
	}
    if(nHeight == 8484)
...
static const int64 nTargetTimespan = 294;	// 84: 7 minutes
static const int64 nTargetSpacing = 84;						// 84: 84 sec
static const int64 nInterval = nTargetTimespan / nTargetSpacing;
static const int64 nTargetTimespanRe = 588; // 14 Minutes
static const int64 nTargetSpacingRe = 84; // 84: 84 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlock *pblock)
{
   ...
   if(fNewDifficultyProtocol) {
    ...
    // 84: This fixes an issue where a 51% attack can change difficulty at will.
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal 84 miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "84", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x4eca3cfb295cb8f84c6dfb52de099c508f18ac5233a95cc10324999fa1898843");
    }
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = " yea ";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04ffff001d01044c534d61792031332c20323031332031313a3334706d204544543a20552e532e2063727564652066757475726573207765726520757020302e332070657263656e74206174202439352e343120612062617272656c") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1389949014;
        ...
        assert(block.hashMerkleRoot == uint256("0x49ddf7fb43604b4cc1dc489ca1ab9c147f316b7fd26c72df3a17cdebf0305147"));
...
{
	return false;
	//commenting out this code until we properly implement 84 alerts.
	/*
    if (!CheckSignature())
        return false;
    if (!IsInEffect())
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // 84: increase each by adding 2 to bitcoin's value.
...
CBlock* CreateNewBlock(CReserveKey& reservekey)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // 84: Reduce the exempted free transactions to 500 bytes (from Bitcoin's 3000 bytes)
